...
import { Calendar, ChevronLeft, Expand, Paperclip } from "lucide-react";
import { getNote, type Note } from "@/lib/store/notes";
...
export default function NoteDetailPage() {
  ...
  if (note === null) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 bg-shell p-6 text-center">
        <p className="text-sm font-semibold text-ink">Note not found</p>
  ...
  if (!note) {
    return <div className="flex h-full items-center justify-center bg-shell text-sm text-muted">Loading</div>;
  }

  // Actually we need to handle loading vs not found distinction: if getNote returns undefined, show not found
  // For simplicity, if note state is null after effect, show not found
  const n = note;
      ...
      <div className="mx-auto w-full max-w-[720px] flex-1 px-6 py-8 md:px-0 md:py-8">
        <article className="overflow-hidden rounded-[24px] bg-white shadow-card ring-1 ring-line/40">
          ...
          <div className="p-6 md:p-10">
            <h1 className="text-[28px] font-bold leading-[1.1] tracking-[-0.03em] text-ink md:text-[26px]">{n.title}</h1>
            ...
            {n.body ? (
              ...
              <p className="mt-6 text-sm leading-relaxed text-muted">No content yet — tap Edit to add your notes.</p>
            )}

            {n.attachments?.length ? (
              <div className="mt-8 border-t border-line/40 pt-6">
                <p className="text-[11px] font-semibold uppercase tracking-widest text-muted">Attachments · {n.attachments.length}</p>
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  {n.attachments.map((a) => (
                    <div key={a.id} className="group relative flex items-center justify-center overflow-hidden rounded-2xl bg-[#f9faf9] p-2 ring-1 ring-line/40">
                      {a.isImage ? (
                        <button type="button" onClick={() => setLightbox(a.dataUrl)} className="group/image relative flex h-[200px] w-full items-center justify-center overflow-hidden rounded-xl bg-white shadow-sm md:h-[220px]">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={a.dataUrl} alt="" className="max-h-full max-w-full object-contain" />
                        ...
                        <div className="flex w-full items-center gap-3 rounded-xl bg-white p-3 ring-1 ring-line">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-[11px] text-muted">{(a.size / 1024).toFixed(0)} KB</p>
                          </div>
                        </div>
                      )}
                    </div>
