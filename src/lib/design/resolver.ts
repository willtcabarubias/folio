...
const FORMAL_RE = /\b(resume|cv|curriculum|cover letter|letter|memo|business plan|proposal|report|executive|manual|certificate|invoice|contract|sop|policy)\b/i;
...
function detectMood(text: string, tone?: string): Mood {
  ...
  if (/\b(minimal|cream|paper|editorial)\b/.test(t)) return "warm";
...
function moodToTheme(mood: Mood, docType: string, tone?: string, format?: string): ThemeId {
  const d = docType.toLowerCase();
  ...
  if (mood === "formal") {
    if (/\bacademic|essay|research|thesis\b/.test(d)) return "academic";
    if (/\b(finance|board|consulting|executive)\b/.test(d)) return "executive";
...
export function resolveDesign(opts: {
  ...
  let themeId = isExplicit ? opts.preferredTheme! : moodToTheme(mood, opts.docType, opts.tone, fmt);
